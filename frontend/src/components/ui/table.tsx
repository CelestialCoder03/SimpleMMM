...
>(({ className, ...props }, ref) => (
  <thead
    ref={ref}
    className={cn(
      "bg-muted/30 sticky top-0 z-10 [&_tr]:border-b",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <th
      ref={ref}
      className={cn(
        "text-left align-middle font-semibold text-muted-foreground tracking-tight [&:has([role=checkbox])]:pr-0",
        density === 'compact' ? "h-10 px-4" : "h-12 px-6",
        className
      )}
      {...props}
    />
