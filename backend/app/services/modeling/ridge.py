"""Ridge regression model with constraint support for Marketing Mix Modeling."""
...
class RidgeModel(BaseModel):
    """
    Ridge regression model with optional coefficient constraints.

    Ridge regression adds L2 regularization to OLS:
        min_β ||y - Xβ||² + α||β||²

    This shrinks coefficients toward zero, reducing variance at the cost
    of some bias. Particularly useful when:
    ...
    Limitations:
        - No automatic variable selection (unlike Lasso)
        - Standard errors require bootstrap or approximation
        - Constraint optimization may be slower
    ...
    Usage:
        ...
        model = RidgeModel(alpha=1.0, constraints=constraints)
        model.fit(X, y, feature_names=['tv', 'radio', 'print'])
    """
    ...
    def _fit_unconstrained(
        ...
        """
        Fit standard Ridge regression without constraints.
        ...
        """
        self._ridge_model = Ridge(
            alpha=self.alpha,
            fit_intercept=self.fit_intercept,
        )
        self._ridge_model.fit(X, y)
        return self._ridge_model.coef_, self._ridge_model.intercept_
    ...
    def _fit_constrained(
        ...
        """
        Fit Ridge regression with coefficient constraints using optimization.
        ...
        def objective(params: NDArray[np.float64]) -> float:
            """Ridge regression objective with L2 penalty."""
            if self.fit_intercept:
                intercept = params[0]
                coef = params[1:]
            else:
            ...
            # Ridge objective: RSS + alpha * ||coef||²
            rss = np.sum((y - y_pred) ** 2)
            penalty = self.alpha * np.sum(coef**2)
        ...
        def gradient(params: NDArray[np.float64]) -> NDArray[np.float64]:
            ...
            # Gradient
            grad_coef = -2 * X.T @ residuals + 2 * self.alpha * coef
    ...
    def get_regularization_path(
        ...
        Useful for visualizing the effect of regularization.
        ...
        Returns:
            Dictionary with 'alphas' and coefficient values per feature.
        """
        if alphas is None:
